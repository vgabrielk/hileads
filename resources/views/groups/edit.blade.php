...
<div class="p-8 space-y-6">
    <!-- Header -->
    <div>
        <a href="{{ route('groups.show', $group) }}" class="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground mb-4 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
            Voltar para Grupo
        </a>
        <h1 class="text-3xl font-bold text-foreground">Editar Grupo</h1>
        <p class="text-muted-foreground mt-1">Modifique as informações do grupo</p>
    </div>
        <form action="{{ route('groups.update', $group) }}" method="POST" id="groupForm">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors bg-background text-foreground">{{ old('description', $group->description) }}</textarea>
                        @error('description')
                            <p class="text-destructive text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                @if($apiError)
                    <div class="bg-warning/10 border border-warning/20 rounded-lg p-4 mb-6">
                        <div class="flex items-center">
                            ...
                            <p class="text-warning-foreground">Não foi possível carregar contatos da API. Você pode adicionar contatos manualmente abaixo.</p>
                        </div>
                    </div>
                @endif
                                    ...
                                    @foreach($apiContacts as $jid => $contact)
                                        <label class="flex items-center p-2 hover:bg-accent/50 rounded-lg transition-colors">
                                            <input type="checkbox" name="contacts[]" value="{{ $jid }}" 
                                                   ...
                                                   {{ in_array($jid, $currentContacts->pluck('contact_jid')->toArray()) ? 'checked' : '' }}>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div class="flex">
                                                    ...
                                                    <input type="text" name="manual_contacts[{{ $index }}][phone]" value="{{ $contact->contact_phone }}" placeholder="11999999999" required
            ...
            <div class="flex items-center justify-end gap-4">
                <a href="{{ route('groups.show', $group) }}" class="px-4 py-2 text-muted-foreground hover:text-foreground font-medium transition-colors">
                    Cancelar
                </a>
...
<script>
let manualContactIndex = {{ count($currentContacts) }};
