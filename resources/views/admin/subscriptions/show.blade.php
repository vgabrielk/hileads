...
<div class="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-foreground">Detalhes da Assinatura</h1>
            <p class="text-muted-foreground mt-1">Visualize e faça a gestão esta assinatura</p>
        </div>
        ...
        <div class="lg:col-span-2 space-y-4 sm:space-y-6">
            ...
            <div class="bg-card rounded-lg border border-border p-4 sm:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-muted-foreground mb-1">ID da Assinatura</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            ...
                            {{ $subscription->user->isAdmin() ? 'Administrador' : 'Usuário' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-card rounded-lg border border-border p-4 sm:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-muted-foreground mb-1">ID da Assinatura Stripe</label>
                ...
                <div class="space-y-2">
                    ...
                    @elseif($subscription->status === 'cancelled')
                        <form method="POST" action="{{ route('admin.subscriptions.reactivate', $subscription) }}" class="inline-block w-full">
                            @csrf
                            <button type="submit" 
                                    onclick="return confirm('Tem certeza que deseja reativar esta assinatura?')"
                    ...
                    <form method="POST" action="{{ route('admin.subscriptions.destroy', $subscription) }}" class="inline-block w-full">
                        ...
                        @method('DELETE')
                        <button type="submit" 
                                onclick="return confirm('Tem certeza que deseja excluir esta assinatura? Esta ação não pode ser desfeita.')"
                                class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-destructive-foreground bg-destructive hover:bg-destructive/90 rounded-lg transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
