                    ...
                    <div class="bg-card rounded-lg border border-border overflow-hidden hover:shadow-md transition-shadow">
                        <div class="p-6">
                            ...
                            <div class="mt-6 flex flex-wrap items-center gap-3 pt-6 border-t border-border">
                                ...
                                <!-- Disconnect Button -->
                                <form method="POST" action="{{ route('whatsapp.disconnect') }}" class="inline" onsubmit="return handleDisconnectConfirmation(event, 'Tem certeza que deseja desconectar?', 'A sessão será mantida e poderá reconectar sem digitalizar o QR Code novamente.')">
                                    @csrf
                                ...
                                <!-- Logout Button -->
                                <form method="POST" action="{{ route('whatsapp.logout') }}" class="inline" onsubmit="return handleLogoutConfirmation(event, 'Tem certeza que deseja fazer logout?', 'Isso irá terminar a sessão e precisará digitalizar o QR Code novamente para ligar.')">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-destructive-foreground bg-destructive hover:bg-destructive/90 rounded-lg transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
